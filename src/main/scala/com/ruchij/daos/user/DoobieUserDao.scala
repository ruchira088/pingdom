...
object DoobieUserDao extends UserDao[ConnectionIO] {
  override def save(user: User): ConnectionIO[Int] =
    sql"""
      INSERT INTO user (id, created_at, modified_at, account_id, first_name, last_name, email)
        VALUES (
          ${user.id},
          ${user.createdAt},
          ${user.modifiedAt},
  ...
  override def findByEmail(email: Email): ConnectionIO[Option[User]] =
    sql"SELECT id, created_at, modified_at, account_id, first_name, last_name, email FROM user where email = $email"
      .query[User]
      .option
}
