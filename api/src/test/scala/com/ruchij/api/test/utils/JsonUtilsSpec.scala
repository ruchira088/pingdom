...
import com.ruchij.api.test.utils.JsonUtils.JsonMismatchError
...
class JsonUtilsSpec extends AnyFlatSpec with Matchers {
  ...
  "contains(Json, Json): Boolean" should "return if the partial JSON is contained within the full JSON" in {
    JsonUtils.contains(json"""{ "name": "John", "age": 10 }""", json"""{ "name": "John" }""") mustBe Right((): Unit)

    JsonUtils.contains(json"""{ "name": "Adam", "age": 10 }""", json"""{ "name": "John" }""") mustBe Left {
      NonEmptyList.of {
        JsonMismatchError(List("name"), Json.fromString("John"), Some(Json.fromString("Adam")))
      }
    }

    JsonUtils.contains(json"""{ "person": { "name": "Adam", "age": 10 } }""", json"""{ "person": { "age": 10 } }""") mustBe Right(
      (): Unit
    )
